...
const validate = (): boolean => {
  ...
  const razon = state.razonSocial.trim() || `${state.nombres} ${state.apellidos}`.trim()
  if (!razon || razon.length < 2) next.razonSocial = 'El nombre o razón social es obligatorio'
...
const handleSubmit = () => {
  ...
  if (!state.razonSocial) {
    state.razonSocial = `${state.nombres} ${state.apellidos}`.trim()
  }
...
<template>
  <AppDashboardPanel id="cliente-create">
    <template #header>
      ...
      <UCard class="max-w-3xl">
        ...
        <form class="grid grid-cols-1 md:grid-cols-2 gap-4" @submit.prevent="handleSubmit">
          ...
          <FormField
            ...
            hint="Cédula/DNI: 6–10 dígitos · RUC: 11–13 dígitos"
          >
          ...
          <FormField label="Razón social / Nombre" name="razonSocial" required :error="errors.razonSocial || errors.razon_social" class="md:col-span-2">
            <UInput v-model="state.razonSocial" placeholder="Se completa con nombres y apellidos si queda vacío" class="w-full" />
          </FormField>
          ...
          <FormField label="Teléfono" name="telefono" required :error="errors.telefono" hint="Exactamente 10 dígitos">
            <UInput v-model="state.telefono" inputmode="numeric" maxlength="10" class="w-full" placeholder="3001234567" />
          </FormField>
